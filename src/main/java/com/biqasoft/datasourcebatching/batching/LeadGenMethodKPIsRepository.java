...
@Service
public class LeadGenMethodKPIsRepository {
    ...
    public IndicatorsDTO getIndicatorsForLeadGenProject(String leadGenProjectId, MongoOperations mongoOperations) {
        ...
        criteria.and("leadGenMethodId").is(leadGenProjectId);
        Query query = new Query(criteria);
        List<CustomerDeal> dealsList = mongoOperations.find(query, CustomerDeal.class);
        ///////////////////////////////////////////

        Criteria criteria2 = new Criteria();
        criteria2.and("leadGenMethodId").is(leadGenProjectId);
        ...
        indicatorsDAO.setType(CUSTOMER_FIELDS.LEAD_GEN_METHOD);
    ...
    private long getCustomersNumberByLeadGenProject(String projectId, MongoOperations mongoOperations) {
        Criteria criteria = new Criteria();
        criteria.and("leadGenMethod").is(projectId).and("customer").is(true);
    ...
    private long getLeadsNumberByLeadGenMethodAndProject(String projectId, MongoOperations mongoOperations) {
        Criteria criteria = new Criteria();
        criteria.and("leadGenMethod").is(projectId).and("lead").is(true);
    ...
    private BigDecimal conversionFromLeadToCustomerByLeadGenMethodAndProject(IndicatorsDTO indicatorsDAO) {
        ...
        Double aDouble = (double) indicatorsDAO.getCustomersNumber() / (double) (indicatorsDAO.getLeadsNumber() + indicatorsDAO.getCustomersNumber());

        BigDecimal result = new BigDecimal(aDouble.toString());
        return result;
    }
    ...
    private BigDecimal leadCostByLeadGenMethod(IndicatorsDTO indicatorsDAO) {
        ...
        Long leadsPlusCustomers = (indicatorsDAO.getLeadsNumber() + indicatorsDAO.getCustomersNumber());

        return indicatorsDAO.getCostsAmount().divide(new BigDecimal(leadsPlusCustomers.toString()), RoundingMode.HALF_UP);
    }
    ...
    private BigDecimal customerCost(IndicatorsDTO indicatorsDAO) {
        ...
        return (indicatorsDAO.getCostsAmount().divide(new BigDecimal(indicatorsDAO.getCustomersNumber()), RoundingMode.HALF_UP));
    }
